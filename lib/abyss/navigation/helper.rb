module Abyss
  ...
  module Navigation
    ...
    module Helper
      ...
      #               Abyss::Navigation.rendering_strategies hash using the
      ...
      def render_navigation(config_path=nil, renderer=nil)
        ...
        target = path.reduce(Abyss::Navigation.configuration) { |acc, g| acc.send g }

        if renderer && renderer.respond_to?(:render)
          target.render(renderer)
        elsif renderer && Abyss::Navigation.rendering_strategies[renderer].nil?
          raise "Rendering strategy not found: #{renderer}"
        elsif renderer
          target.render(Abyss::Navigation.rendering_strategies[renderer])
        else
          target.render
        end
      end
